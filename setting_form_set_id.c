...
static void Paint()
{
    ...
    sprintf(str, "%01x-%01x-%01x-%01x-%01x-%01x",
            g_machineID[0], g_machineID[1], g_machineID[2],
            g_machineID[3], g_machineID[4], g_machineID[5]);
    int displayPosition = (GLCD_WIDTH - GLcd_MeasureString(str)) / 2;
    GLcd_DrawString(str, displayPosition, 35, WHITE);
 ...
 * @retval   NONE
 */
void SettingSetId_Show(uint8_t menuState)
{
 ...
 */
bool SettingSetId_KeyPress(uint8_t keyCode)
{
  /* neu khong hien thi menu thi tra ve false  */
    ...
    else if (keyCode == '*')    /* ENTER key */
    {
        if(controlState == 0x01)
	{
	  /* luu tham so nhiet do can cai dat vao EEPROM khi nhan enter */
